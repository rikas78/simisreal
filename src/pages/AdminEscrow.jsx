    ...
    <div className="space-y-6 max-w-5xl">
      <div>
        <h1 className="font-heading text-2xl md:text-3xl text-foreground flex items-center gap-2">
          <Shield className="w-7 h-7 text-primary" /> Admin — Gestione Escrow
        </h1>
        <p className="text-muted-foreground text-sm mt-1">Montepremi garantiti, freeze e release</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Depositati', value: escrows.filter(e => e.status === 'deposited').length, sub: formatGW(totals.deposited), color: 'text-green-400' },
          { label: 'Congelati', value: escrows.filter(e => e.status === 'frozen').length, sub: formatGW(totals.frozen), color: 'text-orange-400' },
          { label: 'Rilasciati', value: escrows.filter(e => e.status === 'released').length, sub: 'completati', color: 'text-primary' },
        ].map(s => (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={5} className="px-4 py-8 text-center text-muted-foreground">Nessun escrow registrato</td></tr>
              ) : enriched.map(e => {
                const sc = statusConfig[e.status] || statusConfig.pending;
                return (
                  <tr key={e.id} className="border-b border-border/40 hover:bg-secondary/10 transition-colors">
                    <td className="px-4 py-3">
                      <p className="font-semibold text-foreground">{e.race?.title || e.race_title || '—'}</p>
                      <p className="text-[10px] text-muted-foreground">{e.race?.simulator || ''}</p>
                    </td>
                    <td className="px-4 py-3">
                      <p className="font-heading text-base text-primary font-black">{formatGW(e.amount_gw)}</p>
                      <p className="text-[10px] text-muted-foreground">{formatEur(gwToEur(e.amount_gw || 0))}</p>
                    </td>
                    <td className="px-4 py-3">
                      <Badge variant="outline" className={cn("text-[10px]", sc.color)}>{sc.label}</Badge>
                      {e.frozen_reason && <p className="text-[10px] text-orange-400 mt-1">{e.frozen_reason}</p>}
                    </td>
                    <td className="px-4 py-3 text-xs text-muted-foreground">
                      {e.created_date ? format(new Date(e.created_date), 'd MMM yy', { locale: it }) : '—'}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        {e.status === 'deposited' && (
                          <Button size="sm" variant="outline"
                            ...
                            onClick={() => updateEscrow.mutate({ id: e.id, status: 'frozen', reason: 'Freeze admin manuale', raceId: e.race_id })}
                        ...
                        {(e.status === 'frozen' || e.status === 'deposited') && (
                          <Button size="sm" variant="outline"
                            ...
                            onClick={() => updateEscrow.mutate({ id: e.id, status: 'released', raceId: e.race_id })}
                            disabled={updateEscrow.isPending}>
